...
<table class="wp-list-table widefat fixed striped table-view-list users">
  ...
  <tbody>
    <?php foreach ($usersDisplay as $keyUser => $user) {
      ...
      $totalRevenue = 0;
      $totalOrder = 0;
      $userChild = $wpdb->get_results('SELECT * FROM ' . $tableShareLink . ' where status = 2 AND user_parent = ' . $user['ID'], ARRAY_A);
      foreach ($userChild as $child) {
          foreach ($userCommissions as $commission4) {
            if ($commission4['user_id'] === $child['user_id'] && $commission4['status'] == $status['PURCHASE']) {
